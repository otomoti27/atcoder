...
func main() {
	...
	for _, r := range t {
		v := false
		if r == 'U' && cur[0]-1 > 0 && s[cur[0]-1][cur[1]] != "#" {
			cur[0]--
			v = true
		}
		if r == 'D' && cur[0]+1 < h && s[cur[0]+1][cur[1]] != "#" {
			cur[0]++
			v = true
		}
		if r == 'L' && cur[1]-1 > 0 && s[cur[0]][cur[1]-1] != "#" {
			cur[1]--
			v = true
		}
		if r == 'R' && cur[1]+1 < w && s[cur[0]][cur[1]+1] != "#" {
			cur[1]++
			v = true
		}

		if v && !visited[cur[0]][cur[1]] && s[cur[0]][cur[1]] == "@" {
			ans++
